...
export function TemplatePicker({ templates }: TemplatePickerProps) {
  ...
  return (
    <section className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_21rem] lg:items-start">
      ...
      <div className="fixed inset-x-3 bottom-[calc(4.75rem+env(safe-area-inset-bottom,0px))] z-40 rounded-2xl border border-violet-300/20 bg-[#10081c]/95 p-3 shadow-2xl shadow-violet-950/40 backdrop-blur-xl lg:hidden">
        <div className="mb-2 flex items-start gap-2">
          <span className="text-xl leading-none">{selectedTemplate.emoji}</span>
          <div className="min-w-0 flex-1">
            <p className="truncate text-sm font-bold text-white">{selectedTemplate.name}</p>
            <p className="mt-0.5 line-clamp-1 text-xs text-violet-100/70">
              {selectedTemplate.days} days / {paceLabels[selectedTemplate.pace]} / {selectedTemplate.activitiesPerDay}/day
            </p>
          </div>
        </div>
        <Button asChild className="h-10 w-full rounded-xl bg-gradient-to-r from-violet-600 to-indigo-600 shadow-lg shadow-violet-500/25 hover:from-violet-500 hover:to-indigo-500">
