...
export async function startLocalServer({ assetsDirectory } = {}) {
  ...
  try {
    ...
    for (const migration of ["0001_local.sql", "0002_application.sql", "0003_preview_mail.sql", "0004_pilot_catalog.sql", "0005_itineraries.sql", "0007_email_preferences.sql"]) await db.exec(await readFile(join("migrations", migration), "utf8"));
    const names = [
