...
export async function startLocalServer() {
  ...
  try {
    ...
    for (const file of ["index.html", "assets/app.js", "assets/app.css"]) await stat(join(root, "dist/public", file));
    mf = new Miniflare({
      workers: [{ config: {
        ...
        assets: { directory: join(root, "dist/public"), hasUserWorker: true,
          runWorkerFirst: true, notFoundHandling: "single-page-application" },
        env: {
          ...Object.fromEntries(Object.entries({ LOCAL_PROOF: "true", AUTH_BASE_URL: origin,
    ...
    for (const migration of ["0001_local.sql", "0002_application.sql"]) await db.exec(await readFile(join("migrations", migration), "utf8"));
    const names = [
    ...
    await db.batch(names.map(([en, ko, category], i) => db.prepare("INSERT INTO spots VALUES (?, ?, ?, ?, NULL, NULL, 1)")
    ...
    server = createServer({ key: await readFile(key), cert: await readFile(cert) }, async (req, res) => {
      ...
      try {
        ...
        const bounded = new Transform({ transform(chunk, encoding, done) {
          size += chunk.length;
          done(size > 16 * 1024 ? new Error("Request limit") : null, chunk);
        } });
