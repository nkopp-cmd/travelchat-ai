...
test("restricted preview: real RS256, native D1, mocked mail only", { timeout: 180000 }, async t => {
  ...
  try {
    ...
    for (const file of ["0001_local.sql", "0002_application.sql", "0003_preview_mail.sql", "0004_pilot_catalog.sql", "0005_itineraries.sql", "0007_email_preferences.sql", "0008_current_trends.sql", "0009_itinerary_share.sql", "0010_ai_requests.sql"]) await db.exec(await readFile(`migrations/${file}`, "utf8"));
