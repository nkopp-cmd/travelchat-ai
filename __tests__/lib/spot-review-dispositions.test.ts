...
describe("spot review dispositions", () => {
  ...
  it("never approves an ambiguous row for enforcement", () => {
    expect(SPOT_REVIEW_DISPOSITIONS.every((row) => row.enforcementApproved === false)).toBe(true);
    expect(SPOT_REVIEW_DISPOSITIONS.filter((row) => row.decision === "leave_unassigned")).toHaveLength(24);
  });

  it("stages Jiaoxi as an evidenced coordinate fix", () => {
