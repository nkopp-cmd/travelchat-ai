...
import type { PlanningSpot } from "@/lib/itineraries/spot-planning";

type SelectionProps = { selectedSpot?: PlanningSpot | null; spotError?: string | null };
...
function NewItineraryContent({ selectedSpot, spotError }: SelectionProps) {
  const { isLoaded, user } = useUser();
  const identityKey = !isLoaded ? "loading" : user ? `user:${user.id}` : "anonymous";
  ...
  const cities = [...new Set((searchParams.get("cities") || "").split(","))]
    .map(getCityBySlug).filter((city) => city?.isEnabled);
  if (cities.length >= 2) {
    initialData.tripMode = "multi";
    initialData.citySlugs = cities.map((city) => city!.slug);
    initialData.cityNameBySlug = Object.fromEntries(cities.map((city) => [city!.slug, city!.name]));
    initialData.days = parseNumberParam(searchParams.get("days"), MIN_CORRIDOR_DAYS, MAX_CORRIDOR_DAYS) ?? MIN_CORRIDOR_DAYS;
  }

  return <ItineraryWizard key={identityKey} initialData={initialData} initialStep={initialStep} selectedSpot={selectedSpot} spotError={spotError} />;
}

export default function NewItineraryClient(props: SelectionProps) {
  return (
    <Suspense
      fallback={
